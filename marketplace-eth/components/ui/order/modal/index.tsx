...
const createFormState = ({ price, email, confirmationEmail }: IOrder, hasAgreedTOS: boolean, isNewPurchase: boolean) => {
...
const OrderModal: React.FunctionComponent<IOrderModaProps> = ({ course, onClose, onSubmit, isNewPurchase }) => {
  ...
  useEffect(() => {
    if (!!course) {
      setIsOpen(true)
      console.log("eth.perItem", eth.perItem)
      setOrder({ ...defaultOrder, price: Number.parseFloat(eth.perItem.toString()) })
    }
  }, [course, eth.perItem])
    ...
    <Modal isOpen={isOpen}>
      <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
        ...
        <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex">
          <Button
            disabled={ (formState && formState.isDisabled) ?? false }
            onClick={() => {
              onSubmit(order)
            }}>
            Submit
